...
$worker->name = 'MyqqmtsubscribeWorker';
...
$worker->onWorkerStart = function($worker){
    ...
    $mqtt->onMessage = function($topic, $content)use($worker){
        ...
        {
            $conn->send(json_encode(['topic'=>$topic, 'content'=> json_decode($content,true), 'time'=>time(), 'c_id'=>$conn->id]));
        }
    };
    $mqtt->connect();
};
